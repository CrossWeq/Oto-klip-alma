...
anda sohbete otomatik olarak "!clip 60" komutunu gönderir.
...
- "streamer_username": Kendi Kick kanal adınızı yazın. (örn: "LynxCerezLoL")
- "bot_auth_token": Kendi Kick hesabınızın yetki token'ını yazın. 
  ...
  5) F12 ekranında aşağıya yeni satırlar düşecek. Oradan "messages" veya "send" yazan satıra tıklayın.
  6) Sağ tarafta açılan panelde "Headers" (Başlıklar) bölümünü aşağı kaydırın.
  7) "Authorization" isimli satırı bulun. Karşısında yazan ve "Bearer" ile başlayan upuzun şifreyi (Örn: Bearer 333538592|XPNxN...) kopyalayıp config dosyasına yapıştırın.
...
* Yayını Dinleme: Model yüklendikten sonra program canlı yayınınızı arka 
  ...
  ve hemen sohbete "!clip 60" komutu yazar.
